...
Все markdown-артефакты процесса (ТЗ, архитектура, план, задачи, review, отчёты тестирования, открытые вопросы, статус) должны сохраняться в отдельный каталог артефактов `artifacts_dir`, а не в корень проекта.

- По умолчанию используется `docs/implementation`
- Пользователь может задать собственный путь на первом шаге оркестрации
- Оркестратор обязан передавать этот путь во все агентские промпты автоматически
- Дефолтный путь `docs/implementation` добавлен в `.gitignore`; для кастомного пути добавьте соответствующее правило в `.gitignore` проекта
